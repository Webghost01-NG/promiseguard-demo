...
type GithubConnection = { type: 'github-app'; repositories: string[]; installationId: number };
type WorkflowConnection = { type: 'oauth'; label: string };
...
function App() {
  ...
  const [workflowOauth,setWorkflowOauth]=useState<Record<'slack'|'notion',boolean>>({slack:false,notion:false});
  const [workflowConnections,setWorkflowConnections]=useState<Partial<Record<'slack'|'notion',WorkflowConnection>>>({});
  ...
  useEffect(() => {
    const query=new URLSearchParams(location.search);const connectionError=query.get('connection_error');
    if(connectionError)setError(connectionError);
    if(['github','slack','notion'].includes(query.get('connection')||''))setTab('connections');
    if(connectionError||query.has('connection'))history.replaceState({},'',location.pathname);
    fetch('/api/bootstrap').then(r => { if (r.status === 401) { location.reload(); throw new Error('Please sign in again.'); } return r.json(); }).then(data => {
      ...
      setWorkflowOauth(data.workflowOauth||{slack:false,notion:false});setWorkflowConnections(data.workflowConnections||{});
  ...
  return <div className="app-shell">
    ...
    <main id="main-content">
      ...
      {tab === 'connections' && <section className="connections-page"><div className="page-heading"><span className="eyebrow">THE SOURCE OF TRUTH</span><h1>Good decisions.<br/><em>Connected tools.</em></h1><p>GitHub and Gemini get you started. Notion and Slack are optional for each run.</p></div><div className="section-toolbar"><h2>Your integrations <span className="inline-count">04</span></h2><button className="primary" disabled={!session || Boolean(busy)} onClick={() => perform('connections', async () => setConnections(await api('/api/connections', {})))}>{busy === 'connections' ? 'Checking access…' : 'Check connections ↗'}</button></div><div className="connection-grid">{connections.map(c => <article className="connection-card" key={c.provider}><div className="card-top"><Provider name={c.provider}/><span className={`badge ${c.status === 'missing' && ['Notion', 'Slack'].includes(c.provider) ? 'optional' : c.status}`}>{c.status === 'unchecked' ? 'Token saved · unchecked' : c.status === 'missing' && ['Notion', 'Slack'].includes(c.provider) ? 'Not connected · optional' : c.status}</span></div><h3>{c.provider}</h3><span className="eyebrow">{['Notion', 'Slack'].includes(c.provider) ? 'OPTIONAL' : 'REQUIRED'}</span><p>{({GitHub:'The engineering reality. Issues, blockers, and recovery handoffs.',Notion:'The customer promise. Commitments and delivery status.',Slack:'The team context. Discussions and owner notifications.',Gemini:'The reasoning layer. Evidence assessment and proposed changes.'} as Record<string,string>)[c.provider]}</p><div className="connection-detail">{c.detail}</div>{c.provider === 'GitHub' ? <><div className="github-actions"><a className="primary connection-oauth" aria-disabled={Boolean(busy)||running||pending} href={Boolean(busy)||running||pending ? undefined : '/api/connections/github/start'}>{githubConnection ? 'Reconnect GitHub ↗' : 'Connect GitHub ↗'}</a>{githubConnection && <><a className="secondary connection-oauth" aria-disabled={Boolean(busy)||running||pending} href={Boolean(busy)||running||pending ? undefined : '/api/connections/github/repositories'}>Change repository access ↗</a><button className="text-button" disabled={Boolean(busy)||running||pending} onClick={() => perform('github-disconnect',async()=>{await api('/api/connections/github/disconnect',{});setGithubConnection(undefined);setConnections(rows=>rows.map(row=>row.provider==='GitHub'?{...row,status:'missing',detail:'Disconnected from your account.'}:row));})}>{busy==='github-disconnect'?'Disconnecting…':'Disconnect GitHub'}</button></>}</div>{githubConnection && <div className="repository-access"><strong>{githubConnection.repositories.length} selected repositories</strong>{githubConnection.repositories.slice(0,6).map(repo=><span key={repo}>{repo}</span>)}</div>}<details className="token-fallback"><summary>Manual token fallback</summary><ConnectionForm provider={c.provider} disabled={Boolean(busy)||running||pending} save={async token=>{await api('/api/connections/save',{provider:'GITHUB_TOKEN',token});setGithubConnection(undefined);location.reload();}}/></details></> : c.provider !== 'Gemini' && (()=>{const provider=c.provider.toLowerCase() as 'slack'|'notion';const oauthConnection=workflowConnections[provider];const disabled=Boolean(busy)||running||pending;return <><div className="github-actions">{workflowOauth[provider]&&<a className="primary connection-oauth" aria-disabled={disabled} href={disabled?undefined:`/api/connections/${provider}/start`}>{oauthConnection?`Reconnect ${c.provider} ↗`:`Connect ${c.provider} ↗`}</a>}{oauthConnection&&<button className="text-button" disabled={disabled} onClick={()=>perform(`${provider}-disconnect`,async()=>{await api(`/api/connections/${provider}/disconnect`,{});setWorkflowConnections(current=>({...current,[provider]:undefined}));setConnections(rows=>rows.map(row=>row.provider===c.provider?{...row,status:'missing',detail:'Disconnected from your account.'}:row));})}>{busy===`${provider}-disconnect`?'Disconnecting…':`Disconnect ${c.provider}`}</button>}</div>{oauthConnection&&<div className="repository-access"><strong>Connected workspace</strong><span>{oauthConnection.label}</span></div>}<details className="token-fallback" open={!workflowOauth[provider]}><summary>Manual token fallback</summary><ConnectionForm provider={c.provider} disabled={disabled} save={async token=>{const key=({Notion:'NOTION_TOKEN',Slack:'SLACK_BOT_TOKEN'} as Record<string,string>)[c.provider];await api('/api/connections/save',{provider:key,token});setWorkflowConnections(current=>({...current,[provider]:undefined}));setConnections(rows=>rows.map(row=>row.provider===c.provider?{...row,status:token?'unchecked':'missing',detail:token?'Saved for your account. Check access to verify.':'Disconnected from your account.'}:row));}}/></details></>;})()}</article>)}</div><details className="setup-note"><summary>Manage credentials and record access</summary><p>GitHub App access is limited to repositories selected during installation. Reconnect refreshes your authorization without changing repository access. Manual tokens remain available for local testing. Slack and Notion OAuth grants are scoped to the workspace and pages approved during installation. Saved credentials are encrypted and never returned to the browser.</p><p>Gemini is configured by the server operator. Successful authentication does not guarantee access to every page, issue, or channel. Linked records are checked during analysis.</p></details></section>}
...
function VerificationReceipt({run}: {run: Run}) {
  const completion = [...run.events].reverse().find(event => event.text.startsWith('All '));
  return <section className="verification-receipt" aria-label="Verification receipt">
    <div className="receipt-heading"><span className="receipt-check" aria-hidden="true">✓</span><div><span className="eyebrow">VERIFICATION RECEIPT</span><strong>Every approved record matched.</strong></div></div>
    <dl className="receipt-grid">
      <div><dt>RUN ID</dt><dd>{run.id}</dd></div>
      <div><dt>PROVIDERS</dt><dd>{run.actions.map(action => action.provider).join(' · ')}</dd></div>
      <div><dt>VERIFIED</dt><dd>{run.actions.filter(action => action.status === 'verified').length} of {run.actions.length}</dd></div>
      <div><dt>COMPLETED</dt><dd><time dateTime={completion?.at || run.updatedAt}>{new Date(completion?.at || run.updatedAt).toLocaleString()}</time></dd></div>
    </dl>
  </section>;
}
  ...
  return <main className="signin">
    <section className="auth-card" aria-labelledby="auth-title">
      <div className="auth-card-content">
        ...
        <div className="auth-heading"><span className="eyebrow">YOUR COMMITMENTS. YOUR WORKSPACE.</span><h1 id="auth-title">{title}<span className="orange">.</span></h1><p>{signup ? 'Choose a secure way to create your private workspace.' : 'Choose how you want to access your workspace.'}</p><p className="auth-context">PromiseGuard is an open-source hackathon preview that checks commitments against delivery evidence. <a href="https://github.com/Webghost01-NG/promiseguard-demo" target="_blank" rel="noreferrer">View the source</a>.</p></div>
        {state === 'loading' ? <p className="auth-loading" role="status">Opening PromiseGuard…</p> : <>
          ...
          <form className="auth-form" onSubmit={submit}>
            ...
            <label htmlFor="password">Password</label><input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} autoComplete={signup ? 'new-password' : 'current-password'} minLength={8} maxLength={256} placeholder="At least 8 characters" required/>
            {signup && <><label htmlFor="confirm-password">Confirm password</label><input id="confirm-password" type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} autoComplete="new-password" minLength={8} maxLength={256} placeholder="Enter the password again" required/></>}
