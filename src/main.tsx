...
function Auth() {
  ...
  return <main className="signin"><a className="brand" href="/">PromiseGuard<span className="orange">.</span></a><section><span className="eyebrow">YOUR COMMITMENTS. YOUR WORKSPACE.</span><h1>Start here<span className="orange">.</span></h1><p>Use Google, GitHub, or Slack. Your first verified sign-in creates a private workspace automatically.</p>{state === 'loading' ? <p role="status">Opening workspace…</p> : <><div className="social-auth" aria-label="Social sign in">{(['google','github','slack'] as const).map(provider => providers[provider] ? <a className="social-button" href={`/api/auth/${provider}/start`} key={provider}><Provider name={provider[0].toUpperCase()+provider.slice(1)}/><span>Continue with {provider[0].toUpperCase()+provider.slice(1)}</span></a> : <span className="social-button unavailable" aria-disabled="true" key={provider}><Provider name={provider[0].toUpperCase()+provider.slice(1)}/><span>{provider[0].toUpperCase()+provider.slice(1)} setup pending</span></span>)}</div><div className="auth-divider"><span>Invited account</span></div><form onSubmit={async e => {e.preventDefault();setBusy(true);setError('');try {const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,password})});const data=await r.json();if(!r.ok)throw new Error(data.error);setPassword('');setState('ready');}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}><label htmlFor="username">Username</label><input id="username" value={name} onChange={e => setName(e.target.value)} autoComplete="username" maxLength={100} required/><label htmlFor="password">Password</label><input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" maxLength={256} required/>{error && <p className="alert error" role="alert">{error}</p>}<button className="primary full" disabled={busy}>{busy ? 'Signing in…' : 'Sign in ↗'}</button><p className="form-help">Password sign-in remains available for accounts created by the workspace operator.</p></form></>}</section></main>;
}
