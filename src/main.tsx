...
function App() {
  ...
  useEffect(() => {
    fetch('/api/bootstrap').then(r => r.json()).then(data => {
      ...
      setSession(data.session); setTargets(data.targets || initialTargets); setSourceMode(data.targets?.notionPageUrl ? 'notion' : 'github'); setIncludeSlack(Boolean(data.targets?.slackChannel)); setRuns(data.runs);
