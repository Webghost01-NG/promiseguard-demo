...
function App() {
  ...
  useEffect(() => {
    fetch('/api/bootstrap').then(r => r.json()).then(data => {
      ...
      setSession(data.session); setTargets(data.targets || initialTargets); setRuns(data.runs);
  ...
  useEffect(() => {
    ...
    const poll = setInterval(() => {
      fetch('/api/runs').then(r => { if (!r.ok) throw new Error('Could not refresh runs.'); return r.json(); }).then(data => { if (active) setRuns(data); }).catch(() => { if (active) setError('Connection to the local server was interrupted. Saved runs will resume when the server is available.'); });
    }, 2000);
  ...
  async function analyze(e: React.FormEvent) {
    ...
    await perform('analyze', async () => {
      const created = await api('/api/analyze', targets);
  ...
  const runTitle = run?.snapshot?.evidence.find(e => e.provider === 'Notion')?.title || 'Commitment analysis';
  return <div className="app-shell">
    ...
    <header className="app-header">
      ...
      <div className="workspace-identity"><span className="workspace-avatar">W</span><div>Demo workspace<small>Private workspace</small></div></div>
    </header>
    <main id="main-content">
      ...
      {tab === 'workspace' && <>
        <section className="hero">
          ...
          <div className="workflow-map" aria-label="Workflow: Notion, GitHub and Slack evidence is assessed before approval and verification">
            <div className="map-caption"><span className="eyebrow">THREE SOURCES. ONE CLEAR PICTURE.</span><span className="map-index">01 — 03</span></div>
            <div className="map-sources">{sourceNames.map((provider, i) => <div className="map-source" key={provider}><Provider name={provider}/><strong>{provider}</strong><small>{['The promise', 'The progress', 'The context'][i]}</small></div>)}</div>
        ...
        <div className="workspace-grid">
          <aside className="source-panel">
            ...
            {run && !editing ? <>
              ...
              <div className="source-list">{sourceNames.map((provider, i) => { const source = run.snapshot?.evidence.find(e => e.provider === provider); return <div className="source-row" key={provider}><Provider name={provider}/><div><strong>{provider}</strong><small>{source ? `${run.snapshot!.evidence.filter(e => e.provider === provider).length} evidence record${run.snapshot!.evidence.filter(e => e.provider === provider).length === 1 ? '' : 's'} collected` : 'Awaiting collection'}</small></div><External href={source?.url || [activeTargets.notionPageUrl, activeTargets.issueUrl, activeTargets.slackThread][i]}><span className="sr-only">Open {provider} source</span></External></div>; })}</div>
              ...
              <button className="secondary full" onClick={() => { setTargets(run.targets); setThreadCheck(''); setEditing(true); }}>Edit source links <span aria-hidden="true">↗</span></button>
              {pending && <p className="form-help">Finish or dismiss the pending review before analyzing another commitment.</p>}
            </> : <form onSubmit={analyze}>
              <p className="form-intro">Bring the promise and its supporting records together.</p>
              <label htmlFor="issue">GitHub issue URL</label><input id="issue" type="url" placeholder="https://github.com/owner/repo/issues/1" value={targets.issueUrl} onChange={e => field('issueUrl', e.target.value)} required/>
              <label htmlFor="notion">Notion commitment page</label><input id="notion" type="url" placeholder="Paste your Notion page link" value={targets.notionPageUrl} onChange={e => field('notionPageUrl', e.target.value)} required/>
              <label htmlFor="channel">Slack channel URL or ID</label><input id="channel" placeholder="Channel link or C…" value={targets.slackChannel} onChange={e => field('slackChannel', e.target.value)} required/>
      ...
      {tab === 'connections' && <section className="connections-page"><div className="page-heading"><span className="eyebrow">THE SOURCE OF TRUTH</span><h1>Good decisions.<br/><em>Connected tools.</em></h1><p>Three sources of evidence and one reasoning engine.</p></div><div className="section-toolbar"><h2>Your integrations <span className="inline-count">04</span></h2><button className="primary" disabled={!session || Boolean(busy)} onClick={() => perform('connections', async () => setConnections(await api('/api/connections', {})))}>{busy === 'connections' ? 'Checking access…' : 'Check connections ↗'}</button></div><div className="connection-grid">{connections.map(c => <article className="connection-card" key={c.provider}><div className="card-top"><Provider name={c.provider}/><span className={`badge ${c.status}`}>{c.status === 'unchecked' ? 'Token saved · unchecked' : c.status}</span></div><h3>{c.provider}</h3><p>{({GitHub:'The engineering reality. Issues, blockers, and recovery handoffs.',Notion:'The customer promise. Commitments and delivery status.',Slack:'The team context. Discussions and owner notifications.',Gemini:'The reasoning layer. Evidence assessment and proposed changes.'} as Record<string,string>)[c.provider]}</p><div className="connection-detail">{c.detail}</div></article>)}</div><details className="setup-note"><summary>Manage credentials and record access</summary><p>Update tokens in your local <code>.env</code> file, then check connections again. Tokens stay on the server.</p><p>Successful authentication does not guarantee access to every page, issue, or channel. Linked records are checked during analysis.</p></details></section>}
